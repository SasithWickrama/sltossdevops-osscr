$.fn.gantt = function (options) {
    ...
    let dtStart = moment(options.dtStart, "YYYY-MM-DD"); // Define inÃ­cio do calendÃ¡rio
    let dtEnd = moment(options.dtEnd, "YYYY-MM-DD"); // Define fim do calendÃ¡rio
    let countMonth = dtEnd.diff(dtStart, 'month'); // Verifica quantidade de meses entre datas

    let firstDay = '01/'+dtStart.format('MM/YYYY'); // Pega o primeiro dia da data inÃ­cio
    let lastDay = dtEnd.endOf('month').format('DD') +'/'+dtEnd.format("MM/YYYY"); // Pega o Ãºltimo dia da data fim
    ...
    let divGantt = $(this);
	let unic = divGantt.attr('id')+'_'+moment().format('s'); // Cria estancia Ãºnica para minupular tabela
    ...
    //return this.each(function () {
        ...
        if(!options.width){
            ...
            // Redimensiona o grÃ¡fico de acordo com o redimensionamento da tela
		...
		// Defini largura da primeira coluna do header para se ajustar ao layout e nÃ£o ficar correndo para o lado
        if(options.widthCorrectionFirstCol){
			$('.bg-space-clear').css('width',options.widthCorrectionFirstCol+'px');
		}

        // Mapeia todos os IDs de dependÃªncias
        ...
        $.each(tasks, function(index, task) {
            ...
            if(deps.indexOf(task.id.toString()) < 0){ // Pega somentes as atividades ids nÃ£o nÃ£o sÃ£o dependentes
                var tasksTable = '<tr>';
                let classTd = (index % 2 == 0)? 'td-bg1': 'td-bg2';
                for(let i = 0; i <= countDays; i++){
                    //let day = moment(firstDay, "DD/MM/YYYY").add(i, "days").format('DD');
                    let dayNumber = moment(firstDay, "DD/MM/YYYY").add(i, "days").dayOfYear(); // Incrementa o dia
                    if(i == 0){ // Primera interaÃ§Ã£o Ã© o cabeÃ§alho fixo lateral
                        tasksTable += `<td colspan="1" day_number="${dayNumber}"><div></div>${titleName}</td>`;
                    }else{
                        if(d1.dayOfYear()+1 == dayNumber){ // Se nÃºmero do ano baterem em ambas as datas
                            ...
                            if(task.dep){ // Se existe atividades de depedÃªncia, passa para funÃ§Ã£o que monta dependÃªncias
            ...
            // Se existe definiÃ§Ã£o de evento click. Atribui ele ao td da atividade
            if(options.click){
                $('.td-tasks').off('click');
         ...
         * @param {*} ids Ids das atividades que sÃ£o dependentes
         * @param {*} sequence SequÃªncia do dia
         * @param {*} classTd Classe de background-color
         * @param {*} currentDate Data corrente da atividade pai
         * @param {*} firstDay Data inÃ­cio do calendÃ¡rio
         */
        function loadDep(data, ids, sequence, classTd, currentDate, firstDay){
            ...
            $.each(ids.split(','), function(index, id) {
                ...
                $.map(data, function(val, i){
                    // No mapeamento, se o id do json for igual ao id de alguma dependÃªncia e se a data dependÃªncia for maior que data corrente. Monta bloco
                    if(val.id == id && moment(val.date_start, "YYYY-MM-DD").isAfter(moment(currentDate, 'DD/MM/YYYY'))){
                        ...
                        // Continua sequÃªncia de dias a partir da sequencia corrente e data corrente
                        for(var s = sequence; s <= (countDays+sequence); s++){
                            let dayNumberS = moment(firstDay, "DD/MM/YYYY").add(s, "days").dayOfYear();
                            if(d1S.dayOfYear() == dayNumberS){ // Se nÃºmero do ano baterem em ambas as datas
